...
int fuse_read(__attribute__((unused)) const char *path, char *buf, size_t size,
              off_t off, struct fuse_file_info *fi) {
  return pread(fi->fh, buf, size, off) < 0 ? -errno : size;
}

int fuse_write(__attribute__((unused)) const char *path, const char *buf,
...
int fuse_release(__attribute__((unused)) const char *path,
                 struct fuse_file_info *fi) {
  return close(fi->fh) ? -errno : 0;
}

int fuse_fsync(__attribute__((unused)) const char *path, int datasync,
               struct fuse_file_info *fi) {
  if (datasync)
...
int fuse_readdir(__attribute__((unused)) const char *path, void *buf,
                 fuse_fill_dir_t fill, __attribute__((unused)) off_t off,
                 struct fuse_file_info *fi) {
...
int fuse_releasedir(__attribute__((unused)) const char *path,
                    struct fuse_file_info *fi) {
...
void *fuse_init(__attribute__((unused)) struct fuse_conn_info *conn) {
